...
@threestudio.register("zero123-guidance-t")
class Zero123TGuidance(BaseObject):
    @dataclass
    class Config(BaseObject.Config):
        ...
        min_step_percent_coarse: float = 0.02
        max_step_percent_coarse: float = 0.98
        min_step_percent_fine: float = 0.02
        max_step_percent_fine: float = 0.98
    ...
    @torch.cuda.amp.autocast(enabled=False)
    def prepare_embeddings(self, image_dir: str):
        # load cond image for zero123
        rgb_256 = []
        total_image = len(os.listdir(image_dir))
        image_paths = [os.path.join(image_dir, f'{i}.png') for i in range(total_image)]
        ...
        max_batch_size = 8
        total_iter = total_image // max_batch_size
        if total_image % max_batch_size != 0:
            total_iter += 1
        
        c_crossattn_list, c_concat_list = [], []
        for i in range(total_iter):
            rgb_256_ = torch.cat(rgb_256[i*max_batch_size:(i+1)*max_batch_size], dim=0)
            c_crossattn, c_concat = self.get_img_embeds(rgb_256_)
            
            c_crossattn_list.append(c_crossattn)
            c_concat_list.append(c_concat)
        
        self.c_crossattn_list = torch.cat(c_crossattn_list, dim=0)
        self.c_concat_list = torch.cat(c_concat_list, dim=0)
    
    @torch.cuda.amp.autocast(enabled=False)
    def prepare_embeddings_simple(self, image_paths: List):
        image_ids = [int(os.path.basename(path)[:-4]) for path in image_paths]

        self.c_crossattn, self.c_concat = self.c_crossattn_list[image_ids], self.c_concat_list[image_ids]
    ...
    def get_cond(
        ...
        **kwargs,
        ...
        # repeat time
        repeat_time = len(T) if len(self.c_crossattn) != len(T) else 1
        cond = {}
    ...
    def __call__(
        ...
        **kwargs,
        ...
        self.prepare_embeddings_simple(rgb_path)
        
        rgb_BCHW = rgb.permute(0, 3, 1, 2) if not rgb_as_latents else rgb
        ...
        if self.cfg.time_prior is not None:
            ...
            t = torch.randint(
                self.min_step,
                self.max_step + 1,
                [1] if (self.cfg.same_timestamp and mode > 1) else [batch_size],
                dtype=torch.long,
                device=self.device,
            )

            if self.cfg.same_timestamp and mode > 1:
                t = t.repeat(batch_size)

        # predict the noise residual with unet, NO grad!
        with torch.no_grad():
            # add noise
            noise = torch.randn_like(latents) if noise_pre is None else noise_pre # TODO: use torch generator
            latents_noisy = self.scheduler.add_noise(latents, noise, t)
            # pred noise
            x_in = torch.cat([latents_noisy] * 2)
            t_in = torch.cat([t] * 2)
            noise_pred = self.model.apply_model(x_in, t_in, cond, mode=mode, attn_loc=attn_loc)
        ...
        if guidance_eval:
            ...
            guidance_eval_out = {}
        return guidance_out, guidance_eval_out, noise
