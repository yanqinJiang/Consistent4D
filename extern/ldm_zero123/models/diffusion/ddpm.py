...
class LatentDiffusion(DDPM):
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False, mode=1, attn_loc=[]):
        ...
        if hasattr(self, "split_input_params"):
            ...
            x_recon = self.model(x_noisy, t, **cond, mode=mode, attn_loc=attn_loc)
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(
        self, x, t, c_concat: list = None, c_crossattn: list = None, c_adm=None, mode=1, attn_loc=[],
    ):
        if self.conditioning_key is None:
            ...
            xc = torch.cat([x] + c_concat, dim=1)
            cc = torch.cat(c_crossattn, 1)
            out = self.diffusion_model(xc, t, context=cc, mode=mode, attn_loc=attn_loc)
